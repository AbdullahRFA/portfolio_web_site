...
export default function AboutSection({ experiences = [], education = [] }: AboutSectionProps) {
  ...
  // ... (keep your motion variants here)
  const leftFlyInVariants: Variants = { hidden: { opacity: 0, x: -60 }, visible: { opacity: 1, x: 0, transition: { type: "spring", stiffness: 70, damping: 16, delay: 0.1 } } };
  const rightFlyInVariants: Variants = { hidden: { opacity: 0, x: 60 }, visible: { opacity: 1, x: 0, transition: { type: "spring", stiffness: 70, damping: 16, delay: 0.2 } } };
  const parentStaggerVariants: Variants = { hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.12, delayChildren: 0.25 } } };
  const textItemVariants: Variants = { hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0, transition: { duration: 0.4, ease: "easeOut" } } };
  ...
  return (
    <section id="about" className="relative py-24 border-t border-zinc-900 overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
        <motion.div variants={leftFlyInVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="lg:col-span-5 lg:sticky lg:top-24 space-y-6">
          <div className="space-y-2">
            <span className="text-xs font-bold uppercase tracking-widest text-cyan-400">02 . Story Context</span>
        ...
        <motion.div variants={rightFlyInVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="lg:col-span-7 space-y-6">
          <div className="px-2 mb-4">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                <span className="text-xs font-bold uppercase tracking-widest text-fuchsia-400">Chronological Path</span>
                <h3 className="text-2xl font-bold text-zinc-100 mt-1 tracking-tight">Career & Education</h3>
              </div>
              <div className="inline-flex rounded-full bg-zinc-950/70 border border-zinc-800 p-1">
                <button onClick={() => setActiveTab("experience")} className={`rounded-full px-4 py-2 text-xs font-semibold uppercase ${activeTab === "experience" ? "bg-fuchsia-500 text-zinc-950" : "text-zinc-400"}`}>Experience</button>
                <button onClick={() => setActiveTab("education")} className={`rounded-full px-4 py-2 text-xs font-semibold uppercase ${activeTab === "education" ? "bg-emerald-500 text-zinc-950" : "text-zinc-400"}`}>Education</button>
              </div>
            </div>
          </div>
            ...
            {activeTab === "experience" ? (
              <motion.div key="experience" className="space-y-4">
                ...
                {experiences.length > ITEMS_TO_SHOW && (
                  <button onClick={() => setExpandedExp(!expandedExp)} className="mx-auto block text-xs font-bold text-cyan-400 underline">
                    {expandedExp ? "Show Less" : "Show More"}
                  </button>
                )}
              </motion.div>
            ) : (
              <motion.div key="education" className="space-y-4">
                <div className="bg-zinc-900/40 p-6 rounded-3xl border border-zinc-800/80 space-y-4">
                  {education.slice(0, expandedEdu ? education.length : 1).map((item: any, idx: number) => (
                    <motion.article key={item.id || idx} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="p-6 rounded-3xl border border-zinc-800 bg-zinc-950">
                      <h4 className="text-lg font-bold text-zinc-100">{item.institution}</h4>
                      <p className="text-sm text-cyan-400 font-semibold">{item.degree} • {item.date}</p>
                      <p className="text-zinc-400 text-sm mt-2">{item.description}</p>
                      {item.skills && <p className="text-xs text-zinc-500 mt-3 italic">Skills: {item.skills}</p>}
                    </motion.article>
                  ))}
                </div>
                {education.length > 1 && (
                  <button onClick={() => setExpandedEdu(!expandedEdu)} className="mx-auto block text-xs font-bold text-emerald-400 underline">
                    {expandedEdu ? "Show Less" : "View Full Academic Record"}
                  </button>
                )}
              </motion.div>
            )}
