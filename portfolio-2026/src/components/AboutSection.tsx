'use client'; // Required since we weave in scroll-triggered structural motion variants[cite: 1]
...
const AboutSection = () => {
  // 1. Hero Text fly-in from Left side orchestration[cite: 1]
  ...
  // 2. Experience Timeline fly-in from Right side orchestration[cite: 1]
  ...
  // 3. Stagger container list to fade in children bullet layouts sequentially[cite: 1]
  ...
  return (
    <section id="about" className="relative py-24 border-t border-zinc-900 overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
        ...
        {/* --- LEFT COLUMN: BIO WITH FLYING LEFT EFFECT ---[cite: 1] */}
        <motion.div 
          variants={leftFlyInVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: '-100px' }} // Triggers only once precisely when crossing into view track[cite: 1]
        ...
        >
          ...
          >
            ...
            <span className="group-hover:text-cyan-400 transition-colors duration-300">JU CSE Campus Core Member • Class of 2026</span>[cite: 1]
          </motion.div>
        </motion.div>

        {/* --- RIGHT COLUMN: INTERACTIVE HISTORY WITH FLYING RIGHT EFFECT ---[cite: 1] */}
        <motion.div 
        ...
        >
          ...
          <div className="bg-zinc-900/40 p-2 md:p-6 rounded-3xl border border-zinc-800/80 shadow-2xl backdrop-blur-md">
            <Timeline items={experienceData} />[cite: 1]
          </div>
        </motion.div>
